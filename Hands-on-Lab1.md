...
# ファイアウォール (NSG) にてインターネットへのアクセスを制限
...
　NSG サービスタグを利用して、東南アジアリージョンの Azure Storage への接続を許可します。この状態で仮想ネットワークからは、東南アジアリージョンにデプロイされている全ての Azure Storage へアクセスが可能です。他方サービスエンドポイントで接続されている Azure Storage 側は、この仮想ネットワークのサブネットからのみ着信を許可するよう F/W が構成されています。

ここにイメージ

## 特定の Azure Storage のみに接続を許可するようサービスエンドポイントポリシーを適用
